...
import { useAddGAuthUsersMutation } from '../redux/rtk-Api';
import { defaultGAuthUsersData, select, ISelect, gAuthUsersSelectorArr } from '../store/StoreConstants';
...
const AddNextComponents: React.FC = () => {
  const { toggleAddModal, isAddModalOpen, newGAuthUsers, setNewGAuthUsers, setGAuthUsers } = useGAuthUsersStore();
  const [addGAuthUsers, { isLoading }] = useAddGAuthUsersMutation();
  ...
  const handleAddGAuthUsers = async () => {
    const gAuthUsers = {
      dataArr: newItemTags || [],
      name: newGAuthUsers.name || '',
      email: newGAuthUsers.email || '',
      passCode: newGAuthUsers.passCode || '',
      alias: newGAuthUsers.alias || '',
      role: (newGAuthUsers.role as ISelect) || select,
      images: newImages || [],
    ...
    try {
      const addedGAuthUsers = await addGAuthUsers(gAuthUsers).unwrap(); // Get the returned data
      setGAuthUsers([gAuthUsers, addedGAuthUsers]); // Use the returned data instead of the local `GAuthUsers` object
      toggleAddModal(false);
      setNewGAuthUsers(defaultGAuthUsersData);
  ...
  return (
    <Dialog open={isAddModalOpen} onOpenChange={toggleAddModal}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add New GAuthUsers</DialogTitle>
        </DialogHeader>

        <ScrollArea className="h-[400px] w-full rounded-md border p-4">
          <div className="grid gap-4 py-4">
            <InputField id="name" name="name" label="Name" value={(newGAuthUsers.name as string) || ''} onChange={handleInputChange} />
            <InputField id="email" name="email" label="Email" type="email" value={(newGAuthUsers.email as string) || ''} onChange={handleInputChange} />
            <InputField
              id="passCode"
              name="passCode"
              label="Pass Code"
            ...
            />
            <InputField id="alias" name="alias" label="Alias" value={(newGAuthUsers.alias as string) || ''} onChange={handleInputChange} />
