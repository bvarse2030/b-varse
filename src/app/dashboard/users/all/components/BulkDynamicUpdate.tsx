...
const BulkDynamicUpdateNextComponents: React.FC = () => {
  ...
  const { toggleBulkDynamicUpdateModal, isBulkDynamicUpdateModal, bulkData, setBulkData } = useGAuthUsersStore();
        ...
        <ScrollArea className="h-[400px] w-full rounded-md border p-4">
          <div className="flex flex-col gap-2">
            {bulkData.map((GAuthUsers, idx) => (
              <div key={(GAuthUsers._id as string) || idx} className="flex items-start mb-2 justify-between flex-col">
                <div className="flex flex-col">
                  <span>
                    {idx + 1}. {(GAuthUsers.name as string) || ''}
                  </span>
                  {/* <span className="text-xs mt-0">{Array.isArray(GAuthUsers.dataArr) ? gAuthUsers.dataArr.join(', ') : ''}</span> */}
