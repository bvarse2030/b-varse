...
const Page = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        ...
        const url = `http://localhost:3000/dashboard/posts/all/api/v1?id=${id}`;
        if (url) {
          try {
  ...
  return (
    <div className="w-full flex flex-col md:p-4 p-1 gap-4">
      ...
      <Link href="/dashboard/posts/all" className="w-full hover:bg-slate-400 bg-slate-300 p-1 border-1 border-slate-400 ">
        Back to Home
      </Link>
    </div>
  );
