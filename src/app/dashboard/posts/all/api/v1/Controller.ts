...
export async function getPosts(req: Request) {
    ...
    return withDB(async () => {
      ...
      const posts = await Post.find(searchFilter).sort({ updatedAt: -1, createdAt: -1 }).skip(skip).limit(limit);

      const totalPosts = await Post.countDocuments(searchFilter);

      return formatResponse({ posts: posts || [], total: totalPosts, page, limit }, 'Posts fetched successfully', 200);
    });
...
export async function bulkUpdatePosts(req: Request) {
  return withDB(async () => {
    ...
    const successfulUpdates = results.filter(r => r.status === 'fulfilled' && r.value).map(r => (r as PromiseFulfilledResult<typeof Post>).value);
...
export async function deletePost(req: Request) {
  return withDB(async () => {
    ...
    if (!deletedPost) return formatResponse(deletedPost, 'Post not found', 404);
    return formatResponse({ deletedCount: 1 }, 'Post deleted successfully', 200);
  });
}
