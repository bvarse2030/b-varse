...
const BulkEditNextComponents: React.FC = () => {
  ...
  const handleRoleChange = (PostsId: string, role: string) => {
    setBulkData(bulkData.map(Posts => (Posts._id === PostsId ? { ...Posts, role } : Posts)) as IPosts[]);
  };
            ...
            {bulkData.map((Posts, idx) => (
              <div key={(Posts._id as string) || idx} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 min-w-[180px]">
                  ...
                  <Select onValueChange={role => handleRoleChange(Posts._id as string, role)} defaultValue={(Posts.role as string) || ''}>
